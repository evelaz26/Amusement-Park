 ...
 * 
 * This file contains code to create and draw a cylinder object.
 *  
*/
class Cylinder{ 

	constructor( nSectors, gl, program, color, texture) {
		
        this.nSectors = nSectors;       // The number of triangles to draw for the cylinder
		...
		for( i = 0; i < nSectors + 1; i++ ) { // Duplicate the starting point
			...
			// The same normal for each vertex of the triangle, so we reuse thrice
			...
			texindex.push(texture); texindex.push(texture); // Display the checkered texture
		}	
